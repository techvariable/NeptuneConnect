...
const settings: FastifyPluginAsync = async (fastify, opts): Promise<void> => {
  fastify.get(
    ...
    async function (request, reply) {
      return await reply.view('/view/editor.ejs', { url: `${process.env.APP_URL}api/editor`, imgurl: `${process.env.APP_URL}`, user: JSON.stringify({ name: request.user?.name, email: request.user?.email }) })
    }
  )
}
