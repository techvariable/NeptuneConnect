...
const settings: FastifyPluginAsync = async (fastify: FastifyInstance, opts): Promise<void> => {
  fastify.get(
    ...
    async function (request: FastifyRequest, reply: FastifyReply) {
      ...
      return await reply.view('/view/apiKey.ejs', { navigators, url: `${process.env.APP_URL}api/settings`, imgurl: `${process.env.APP_URL}`, permissions: request.user?.permissions, user: JSON.stringify({ name: request.user?.name, email: request.user?.email }), mode: process.env.MODE ?? 'prod' })
    }
  )
}
